...
async function test() {
  console.log("Testing orchestrator with 'the housemaid'...");
  ...
  // If we just pass title="the housemaid", getStreams takes an ID. 
  ...
  try {
    const results = await tmdb.searchCatalog('movie', 'the housemaid');
    if (results.length > 0) {
      ...
      console.log("No results for 'the housemaid' in TMDB.");
    }
  } catch (err) {
    console.error(err);
  }
